...
function verifyLink(link) {
	...
	if (linkHref !== link.innerHTML) {
		return confirm('You want to go to ' + linkHref + ',right?');
	}
...
"Welcome to XClient.\nThis is a client for chatting rooms of Hack.Chat series based on XChat client.\n"+
"This is a random room for you:  ?" +Math.random().toString(36).substr(2, 8)+
"\n-----\n[Click me to visit our website](https://www.zzchat.cf).\n"+
"You can type a WebSocket path of the chatting room you want to connect.\n"+
"XClient is based on XChat: https://gitee.com/liguiyu102210/xchat  \n"+
"XChat is based on Hack.Chat: https://github.com/hack-chat/  \n"+
"**If you want to use XClient for business,please mark the authors of this project (Gitee@liguiyu102210 and bilibili@智障初墨).** \n-----\n"+
...
function RequestNotifyPermission() {
	try {
				...
				if (result === "granted") {
					if (notifyPermissionExplained === 0) {
						pushMessage({
							cmd: "chat",
							nick: "*",
							text: "You can receive notifications now.",
							time: null
						});
		...
		pushMessage({
			cmd: "chat",
			nick: "*",
			text: "Can't create notifications.",
			time: null
		});
...
function join(channel) {
	...
	if(localStorageGet('customChannel') != null) {
		var customWsPath=prompt('Please type a WebSocket path (ws:// or wss://):', localStorageGet('customWsPath'));
	}else{
		var customWsPath=prompt('Please type a WebSocket path (ws:// or wss://):');
	}
	if(customWsPath != '' && customWsPath!='null' && customWsPath!=null) {
		...
		alert('Please type a WebSocket path.');
	}
	...
	ws.onopen = function () {
		...
		if (myNick && shouldConnect) {
			...
			pushMessage({ nick: '*', text: "Users online: " + a_nicks.join(", ") })
		}
		var h5_mynick = document.querySelector("#mynick")
		h5_mynick.innerHTML = 'Your nickname：'+myNick.split('#')[0]
...
var COMMANDS = {
	...
	kill: function (args) {
		...
		pushMessage({ nick: '*', text: "Kicked"+args.nick});
	},
